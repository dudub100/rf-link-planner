...
def calculate_itu_capacity(lat, lon, d_km, f_ghz, tx_p, g_a, g_b, t_c, rh, bw_mhz, nf, max_qam):
    ...
    for avail in [99.0, 99.9, 99.99, 99.999]:
        ...
        cap_mbps = bw_mhz * min(np.log2(1 + 10**(max(0.1, 10**(eff_snr/10)))), max_spec_eff) if eff_snr > -10 else 0
...
if col_gps.button("📍 Set My GPS"): st.session_state.gps_requested = True; st.rerun()
if col_wea.button("☁️ Sync Weather"):
...
# WhatsApp Logic
curr_url = streamlit_js_eval(js_expressions="window.parent.location.href", want_output=True, key="get_parent_url")
if curr_url:
    ...
    wa_link = f"https://wa.me/?text={urllib.parse.quote('Sync your Site B with me: ' + s_url)}"
    st.sidebar.markdown(f'''<a href="{wa_link}" target="_blank" style="text-decoration:none;"><div style="background-color:#25D366;color:white;padding:12px;border-radius:8px;text-align:center;font-weight:bold;">📲 Share Site A with Peer</div></a>''', unsafe_allow_html=True)
...
st.sidebar.divider()
st.sidebar.subheader("RF & System")
...
qam_opts = [2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384]
...
st.session_state.env_temp = st.sidebar.number_input("Temp (°C)", value=float(st.session_state.env_temp), format="%.1f")
st.session_state.env_rh = st.sidebar.number_input("Humidity (%)", value=float(st.session_state.env_rh), format="%.1f")

# --- 4. MAIN UI ---
st.title("RF Link Path Profiler & Capacity Tool")
...
with c2:
    if st.button("🚀 Analyze Link & Capacity", type="primary", use_container_width=True):
        ...
        if df is not None:
            ...
            diff = np.abs(ang_a_ray - ang_b_ray + 2 * slope_ang); diff[~( (x>0) & (x<dist) )] = np.inf
            ...
            r_ratio = 10**(-disc/20); rsl_dest = rsl_c + 20*np.log10(max(1e-4, 1-r_ratio))
            params = {
                "dist_km": dist/1000, "pdf_status": "OBSTRUCTED" if is_obs else "CLEAR", "ref_disc": disc, "rsl_dest": rsl_dest, "rsl_const": rsl_c + 20*np.log10(1+r_ratio),
