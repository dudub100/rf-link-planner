...
def generate_pdf_report(p, profile_img_path, df_att):
    try:
        ...
        pdf.set_font("helvetica", "B", 11); pdf.cell(0, 8, "1. Site & Antenna Details", ln=True)
        pdf.set_font("helvetica", "", 9)
        pdf.cell(0, 6, f"Distance: {p['dist_km']:.3f} km | Status: {p['pdf_status']} | Freq: {p['freq']} GHz | BW: {p['bw']} MHz", ln=True)
        pdf.cell(0, 6, f"Site A: {p['lat_a']:.5f}, {p['lon_a']:.5f} | H: {p['h_a']}m | Dish: {p['d_a']}m ({p['g_a']:.1f} dBi, BW: {p['bw_a']:.2f} deg)", ln=True)
        pdf.cell(0, 6, f"Site B: {p['lat_b']:.5f}, {p['lon_b']:.5f} | H: {p['h_b']}m | Dish: {p['d_b']}m ({p['g_b']:.1f} dBi, BW: {p['bw_b']:.2f} deg)", ln=True)
        ...
        if profile_img_path: pdf.ln(3); pdf.set_font("helvetica", "B", 11); pdf.cell(0, 8, "3. Terrain Profile", ln=True); pdf.image(profile_img_path, x=10, w=185); pdf.ln(5)
...
if curr_url:
    ...
    st.sidebar.markdown(f'''<a href="{wa_link}" target="_blank" style="text-decoration:none;"><div style="background-color:#25D366;color:white;padding:10px;border-radius:8px;text-align:center;font-weight:bold;">📲 Share Site A with Peer</div></a>''', unsafe_allow_html=True)
...
with col2:
    ...
    if st.session_state.results_ready:
        with st.status("Performing Engineering Analysis...") as status:
            ...
            if df is not None:
                ...
                st.markdown(f"**Clear Sky RSL:** `{rsl_clr:.1f} dBm` | **Worst-Case Multipath:** `{rsl_dest:.1f} dBm` | **Diffraction Loss:** `{diff_loss:.1f} dB`")
                ...
                try:
                    with tempfile.TemporaryDirectory() as tmp:
                        ...
                        st.session_state.pdf_data = generate_pdf_report({"dist_km": dist/1000, "pdf_status": "OBSTRUCTED" if is_obs else "CLEAR", "freq": freq, "bw": st.session_state.ch_bw, "rsl_clear": rsl_clr, "diff": diff_loss, "ref_disc": disc, "rsl_dest": rsl_dest, "lat_a": st.session_state.lat_a, "lon_a": st.session_state.lon_a, "h_a": st.session_state.h_a, "g_a": g_a, "d_a": d_a, "bw_a": bw_a, "lat_b": st.session_state.lat_b, "lon_b": st.session_state.lon_b, "h_b": st.session_state.h_b, "g_b": g_b, "d_b": d_b, "bw_b": bw_b, "fsl": fsl, "gas": gas}, img_p, df_att)
                except: st.session_state.pdf_data = generate_pdf_report({"dist_km": dist/1000, "pdf_status": "OBSTRUCTED" if is_obs else "CLEAR", "freq": freq, "bw": st.session_state.ch_bw, "rsl_clear": rsl_clr, "diff": diff_loss, "ref_disc": disc, "rsl_dest": rsl_dest, "lat_a": st.session_state.lat_a, "lon_a": st.session_state.lon_a, "h_a": st.session_state.h_a, "g_a": g_a, "d_a": d_a, "bw_a": bw_a, "lat_b": st.session_state.lat_b, "lon_b": st.session_state.lon_b, "h_b": st.session_state.h_b, "g_b": g_b, "d_b": d_b, "bw_b": bw_b, "fsl": fsl, "gas": gas}, None, df_att)
