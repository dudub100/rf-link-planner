...
# Ensure state persistence for every single feature
defaults = {
...
# --- 2. IMPROVED BATCHED ELEVATION FUNCTION ---
def get_elevation_profile(lat1, lon1, lat2, lon2, num_points=100):
    ...
    elevs = []
    batch_size = 40  # Smaller batches to prevent API timeouts
    for i in range(0, len(coords), batch_size):
        batch = coords[i:i+batch_size]
        loc_str = "|".join([f"{lat},{lon}" for lat, lon in batch])
        try:
            # Attempt Mapzen first, fallback to SRTM30m
...
def calculate_itu_capacity(lat, lon, d_km, f_ghz, tx_p, g_a, g_b, t_c, rh, bw_mhz, nf, max_qam, diff_loss):
    ...
    for avail in [99.0, 99.9, 99.99, 99.999]:
        ...
        cap = bw_mhz * min(np.log2(1 + 10**(max(-5, eff_snr_now)/10)), np.log2(max_qam)) if eff_snr_now > -10 else 0
...
# --- 3. SIDEBAR (ALL FEATURES RESTORED) ---
...
# --- 4. MAIN UI (ALL VISUALS RESTORED) ---
    ...
    if st.session_state.results_ready:
        with st.status("Performing Engineering Analysis...") as status:
            ...
            if df is not None:
                ...
                df_att, fsl, gas, rsl_clr = calculate_itu_capacity
